...
def getLastData():
    ...
    return read_time,temp, hum

@app.route('/')
def index():
	read_time,temp, hum = getLastData()
	templateData = {
        'read_time': read_time,
        'temp'  : temp,
        'hum'	: hum
...
def getRecentMeasurements():
    ...
    return render_template("Graphs.html",labels=labels, values=values,labels_hum=labels_hum, values_hum=values_hum)
